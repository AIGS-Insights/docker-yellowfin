...
LABEL description="Yellowfin ${APP_VERSION}"
...
COPY assets/yellowfin-${APP_VERSION}-${APP_BUILD}-full.jar /tmp/yf-install/yellowfin.jar

COPY assets/openjfx-18.0.1_linux-x64_bin-sdk.zip /tmp/yf-install/javafx.zip
...
COPY assets/default.properties /tmp/yf-install/custom.properties
...
COPY assets/docker_configuration.sh /opt/yellowfin/
